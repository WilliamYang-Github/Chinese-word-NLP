 ...
 *
 * <copyright file="TransformMatrixDictionary.java">
 ...
 * @author hankcs
 */
public class SecondOrderHiddenMalkovModel<E extends Enum<E>>
{
    Class<E> enumType;
    /**
    ...
    public SecondOrderHiddenMalkovModel(Class<E> enumType)
    {
    ...
    public boolean load(String path)
    {
        try
        {
            ...
            ++ordinaryMax;
            bigramMatrix = new int[ordinaryMax][ordinaryMax];
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                for (int i = 0; i < ordinaryArray.length; ++i)
                {
                	bigramMatrix[currentOrdinary][ordinaryArray[i]] = Integer.valueOf(paramArray[1 + i]);
                }
            }
            br.close();
            // 需要统计一下每个标签出现的次数
            ...
            for (int from : states)
            {
                for (int to : states)
                {
                    double frequency = bigramMatrix[from][to] + 1e-8;
...
//                    System.out.println("from" + NR.values()[from] + " to" + NR.values()[to] + " = " + transititon_probability[from][to]);
    ...
    {
        return bigramMatrix[from.ordinal()][to.ordinal()];
    }
    ...
    public void extendSize()
    {
        ...
        {
            System.arraycopy(bigramMatrix[i], 0, n_matrix[i], 0, bigramMatrix.length);
        }
        bigramMatrix = n_matrix;
    }
    ...
    public String toString()
    {
        ...
        sb.append(", matrix=").append(Arrays.toString(bigramMatrix));
