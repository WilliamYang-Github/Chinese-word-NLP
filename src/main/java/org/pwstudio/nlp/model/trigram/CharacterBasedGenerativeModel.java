 ...
 * </copyright>
 */
package org.pwstudio.nlp.model.trigram;
...
import org.pwstudio.nlp.model.trigram.frequency.Probability;
...
public class CharacterBasedGenerativeModel implements ICacheAble
{
    ...
    double log_prob(char s1, int i1, char s2, int i2, char s3, int i3)
    {
        ...
        if (uni + bi + tri == 0)
            return inf;
        double result = Math.log(uni + bi + tri);
        return result;
}
    ...
    public char[] tag(char[] charArray)
    {
        ...
        double[][] now = new double[tagCount][tagCount];
        double[] first = new double[tagCount];

        // link[i][s][t] := 第i个节点在前一个状态是s，当前状态是t时，前2个状态的tag的值
        int[][][] link = new int[charArray.length][tagCount][tagCount];
        // 第一个字，只可能是bs
        for (int s = 0; s < tagCount; ++s)
        {
        ...
        // 第三个字开始，利用TriGram标注
        double[][] pre = new double[tagCount][tagCount];
    ...
    {
        if (table.v.length == 0) return;
        if (table.v.length == 1)
        {
            table.v[0][2] = "S";
            return;
        }
        char[] charArray = new char[table.v.length];
        for (int i=0; i<charArray.length; i++)
            charArray[i] = table.v[i][0].charAt(0);
    //    char[] tag = new char[table.v.length];
        double[][] now = new double[tagCount][tagCount];
        double[] first = new double[tagCount];

        // link[i][s][t] := 第i个节点在前一个状态是s，当前状态是t时，前2个状态的tag的值
        int[][][] link = new int[charArray.length][tagCount][tagCount];
        // temp[i][l][2] := 计数临时变量 temp[i][l][0] = tf.get(si,id2tag[i])
        //                                 temp[i][l][1] = tf.get(si-1,t1-1, si,id2tag[i])  
        int[][][] temp = new int[charArray.length][tagCount][tagCount+1];
        // 第一个字，只可能是bs
        for (int f = 0; f < tagCount; ++f)
        {
            double p = (f == 1 || f == 2) ? inf : log_prob(bos[0], 4, bos[0], 4, charArray[0],f);
            first[f] = p;
        }
        ...
        // 第三个字开始，利用TriGram标注
        double[][] pre = new double[tagCount][tagCount];
        for (int i = 2; i < table.v.length; i++)
        {
            // swap(now, pre)
            double[][] swap = pre;
            pre = now;
        ...
        t = now[s][3] > now[s][2] ? 3 : 2;
        for (int i = link.length - 1; i >= 0; --i)
        {
            table.v[i][2] = String.valueOf(id2tag[t]);
            int f = link[i][s][t];
            t = s;
            s = f;
        }
    ...
    public boolean load(ByteArray byteArray)
    {
        ...
        for (int i = 0; i < id2tag.length; i++)
        {
            ...
            {
                for (int k = 0; k < id2tag.length; k++)
                {
                    transMatrix[i][j][k] = tf.get(id2tag[i], id2tag[j], id2tag[k]);
                }
            }
        }
        return true;
