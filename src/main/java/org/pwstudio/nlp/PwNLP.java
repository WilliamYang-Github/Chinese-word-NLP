...
import org.pwstudio.nlp.summary.TextRankKeyword;
import org.pwstudio.nlp.summary.TextRankSentence;
import org.pwstudio.nlp.tokenizer.StandardTokenizer;
import org.pwstudio.nlp.tokenizer.mixed.CHMMSegment;
import org.pwstudio.nlp.utility.Predefine;
import org.pwstudio.nlp.utility.TextUtility;

/**
 * PwNLP: Han Language Processing <br>
 * 汉语言处理包 <br>
 * 常用接口工具类
 *
 * @author hankcs
...
{
    /**
     * 库的全局配置，既可以用代码修改，也可以通过pwnlp.properties配置（按照 变量名=值 的形式）
     */
    public static final class Config
    {
         ...
         * CRF分词模型
         */
        // @deprecated 已废弃，请使用{@link com.hankcs.pwnlp.model.crf.CRFLexicalAnalyzer}。未来版本将不再发布该模型，并删除配置项
        ...
        /**
         * IO适配器（默认null，表示从本地文件系统读取），实现com.hankcs.pwnlp.corpus.io.IIOAdapter接口
         * 以在不同的平台（Hadoop、Redis等）上运行PwNLP
         */
        public static IIOAdapter IOAdapter;

        /**
         * 领域的附加子模型文件（父路径、词典、邻接矩阵）
         */
        public static String customRoot = "data/dictionary/custom";
        
        public static String defaultUnigramName = "NatureDict.txt";
        
        public static String defaultBigramName = "NatureDict.ngram.txt";
        ...
        static
        {
            ...
            try
            {
                ...
                {  // IKVM (v.0.44.0.5) doesn't set context classloader
                    loader = PwNLP.Config.class.getClassLoader();
                }
                p.load(new InputStreamReader(Predefine.HANLP_PROPERTIES_PATH == null ?
                                                 loader.getResourceAsStream("pwnlp.properties") :
            ...
            {
                if (Predefine.HANLP_ROOT_PATH != null)
                    root = Predefine.HANLP_ROOT_PATH;
                else
                {
                    StringBuilder sbInfo = new StringBuilder("========Tips========\n请将pwnlp.properties放在下列目录：\n"); // 打印一些友好的tips
                    ...
                    sbInfo.append("现在PwNLP将尝试从").append(System.getProperties().get("user.dir")).append("读取data……");
                    logger.severe("没有找到pwnlp.properties，可能会导致找不到data\n" + sbInfo);
                }
            }
    ...
    /**
     * 简转繁,是{@link org.pwstudio.nlp.PwNLP#convertToTraditionalChinese(java.lang.String)}的简称
     *
     * @param s 简体中文
     * @return 繁体中文(大陆标准)
     */
    ...
    {
        return PwNLP.convertToTraditionalChinese(s);
    }

    /**
     * 繁转简,是{@link PwNLP#convertToSimplifiedChinese(String)}的简称
     *
     * @param t 繁体中文(大陆标准)
     * @return 简体中文
     */
    ...
    {
        return PwNLP.convertToSimplifiedChinese(t);
    }
     ...
     * 创建一个分词器<br>
     * 这是一个工厂方法<br>
     * 与直接new一个分词器相比，使用本方法的好处是，以后PwNLP升级了，总能用上最合适的分词器
     *
     * @return 一个分词器
     */
    public static Segment newSegment()
    {
        return new CHMMSegment();   // Viterbi分词器是目前效率和效果的最佳平衡
    }
