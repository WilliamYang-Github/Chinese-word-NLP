 ...
 *
 * <copyright file="WordDictionary.java">
...
public class CustomDictionary
{
    ...
    static
    {
        String path[] = PwNLP.Config.CustomDictionaryPath;
    ...
    public static boolean load(String path, Nature defaultNature, TreeMap<String, CoreDictionary.Attribute> map, LinkedHashSet<Nature> customNatureCollector)
    {
        try
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                if (PwNLP.Config.Normalization) param[0] = CharTable.convert(param[0]); // 正规化
...
//                if (updateAttributeIfExist(param[0], attribute, map, rewriteTable)) continue;
    ...
    {
        if (PwNLP.Config.Normalization) word = CharTable.convert(word);
        if (contains(word)) return false;
    ...
    {
        if (PwNLP.Config.Normalization) key = CharTable.convert(key);
